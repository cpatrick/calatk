...
1)  Compile CALATK with RelWithDebInfo CMAKE_BUILD_TYPE

2)  cd into  the CALATK binary build directory
...
For example:
    ...
    ctest  -R   calatkRegularizationSpacingTest  -N

This will print the tests selected by the regular expression but not run the
tests (-N option).

4)  Type the path to this script in the CALATK source tree and add the select expression
    from step 3 above.

For example:

   ~/src/CALATK/Scripts/runValgrind.sh    \
      -R calatkRegularizationSpacingTest
