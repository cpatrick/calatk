...
%             to be cropped.
%   -m<value> - optional string where value indicates the factor to magnify
%               the on-screen figure dimensions by when generating bitmap
%               outputs. Default: '-m1'.
%   -a1, -a2, -a3, -a4 - optional sting indicating the amount of anti-
%                        aliasing to use for bitmap outputs. '-a1' means no
%                        anti-aliasing; '-a4' is the maximum amount
%                        (default).
...
%   -cmyk - optional string indicating color figures should be saved in the
%           CMYK colorspace rather than the RGB colorspace (default). Only
...
% $Id: export_fig.m,v 1.21 2009/10/09 09:46:02 ojw Exp $
...
options.magnify = options.magnify * options.aa_factor;
if isbitmap(options) && options.magnify ~= 1
...
if isbitmap(options)
    ...
    if isequal(tcol, 'none') && (options.png || options.alpha)
        ...
        B = print2array(fig, options.magnify, options.zbuffer);
        % Downscale the image
        B = downsize(single(B), 0, options.aa_factor);
        % Set background to white
        set(fig, 'Color', 'w');
        % Print large version to array
        A = print2array(fig, options.magnify, options.zbuffer);
        % Downscale the image
    ...
    elseif ischar(varargin{a}) && ~isempty(varargin{a})
        if varargin{a}(1) == '-'
            switch lower(varargin{a}(2:end))
                ...
                otherwise
                    options.magnify = str2double(regexp(varargin{a}, '(?<=-(m|M))(\d*\.)?\d+(e-?\d+)?', 'match'));
                    if ~isscalar(options.magnify)
